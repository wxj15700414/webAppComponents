<template>
	<view class="zzx-calendar">
		<view class="calendar-heander">
			<view>{{ formatDate(new Date(selectedDate), 'yyyy.MM.dd') }}</view>
			<view @click="goback" class="backToday">返回今天</view>
		</view>
		<view class="calendar-weeks">
			<view class="calendar-week" v-for="(week, index) in weeks" :key="index">{{ week }}</view>
		</view>
		<view class="calendar-content">
			...
			>
				<swiper-item class="calendar-item" v-for="sitem in swiper" :key="sitem">
					<view class="calendar-days">
						<view class="bg-month">{{ bgMonth }}</view>
						<template v-if="sitem === current">
							<view class="calendar-day" v-for="(item, index) in days" :key="index" :class="{ 'day-hidden': !item.show }" @click="clickItem(item)">
								...
								<view class="dot-show" v-if="item.info" :style="[dotStyle]" :class="[item.fullDate === selectedDate ? 'selectDay' : '']"></view>
							</view>
						</template>
						<template v-else>
			...
			<view class="mode-change" @click="changeMode">
				<icon-font v-if="weekMode" icon="iconunflod" color="#dcdfe6;"></icon-font>
				<icon-font v-if="!weekMode" icon="iconflod" color="#dcdfe6;"></icon-font>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	computed: {
		...
		timeStr() {
			...
			const day = d.getDay() + 1 <= 9 ? `0${d.getDay() + 1}` : d.getDay() + 1
			str = `${y}.${m}.${day}`
			return str
		},
		predays() {
	...
	methods: {
		...
		initDate(cur) {
			...
			this.bgMonth = this.currentMonth
		...
		goback() {
			...
			this.initDate(d)
			this.selectedDate = formatDate(new Date(), 'yyyy-MM-dd')
			this.$emit('selected-change', formatDate(new Date(), 'yyyy-MM-dd'))
		},
		goDate(date) {
			const d = new Date(date)
			this.initDate(d)
			this.selectedDate = formatDate(new Date(date), 'yyyy-MM-dd')
			this.$emit('selected-change', formatDate(new Date(date), 'yyyy-MM-dd'))
		}
	},
	created() {
		this.initDate()
